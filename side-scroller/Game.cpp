...
void Game::keyPressEvent(QKeyEvent *e) {
  ...
  if (e->key() == Qt::Key_Q) {
	...
	bullet->setPos(player->pos().x() + player->boundingRect().width() + 15,
	               player->pos().y() + player->boundingRect().height() / 3);
      } else {
	bullet->setPos(player->pos().x() - bullet->boundingRect().width() - 15,
	               player->pos().y() + player->boundingRect().height() / 3);
      }
  ...
  if (e->key() == Qt::Key_W) {
    if (player->rocket_interval > 60) {
      ...
      if (player->getDir() == RIGHT) {
	rocket->setPos(player->pos().x() + player->boundingRect().width() + 15,
	               player->pos().y() + player->boundingRect().height() / 3);
      } else {
	rocket->setPos(player->pos().x() - rocket->boundingRect().width() - 15,
	               player->pos().y() + player->boundingRect().height() / 3);
      }
  ...
  if (e->key() == Qt::Key_Right || e->key() == Qt::Key_Left) {
    ...
    player->setDirection(e->key() == Qt::Key_Right ? Direction::RIGHT
                                                   : Direction::LEFT);
  }
